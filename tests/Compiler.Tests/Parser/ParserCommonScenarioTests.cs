...
""";
    ...
    [Fact]
    public void ParsesGenericArrayConstructors()
    {
        var source = @"
class Collections is
    method Build is
        var numbers : Array[Integer](10)
        var reals : Array[Real](5)
    end
end
";
        ...
        var realArrayCtor = Assert.IsType<ConstructorCallNode>(locals[1].InitialValue);
        Assert.Equal("Array", realArrayCtor.ClassName);
        Assert.NotNull(realArrayCtor.GenericArgument);
        Assert.Equal("Real", realArrayCtor.GenericArgument!.Name);
    }
